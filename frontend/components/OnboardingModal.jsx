...
import { X, CheckCircle, ArrowRight, Star, Shield } from 'lucide-react';
import { Dialog, DialogContent } from './ui/dialog';
import { Button } from './ui/button';
import { Card } from './ui/card';
import { Badge } from './ui/badge';
export function OnboardingModal({ open, onOpenChange, onOnboardingComplete, mode = 'first-login' }) {
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-none w-screen h-screen p-0 bg-background border-0 overflow-hidden">
        <div className="relative w-full h-full overflow-y-auto">
          ...
          {/* Close button */}
          <button onClick={() => onOpenChange(false)} className="fixed top-6 right-6 z-50 p-3 rounded-xl hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
            <X className="h-6 w-6"/>
          </button>
          ...
          <div className="min-h-full flex flex-col items-center justify-center px-6 py-12">
            ...
            {isChannelConnection && (<div className="w-full max-w-5xl mb-12">
            ...
            <div className="w-full max-w-7xl mb-12">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {plans.map((plan) => (<Card key={plan.id} className={`relative p-8 cursor-pointer transition-all duration-300 border-2 min-h-[600px] flex flex-col ${selectedPlan === plan.id
                ? 'border-primary shadow-xl shadow-primary/10 scale-[1.02]'
                : plan.popular
                    ? 'border-primary/20 shadow-lg'
                    : 'border-border hover:border-muted-foreground/30 hover:shadow-lg'}`} onClick={() => setSelectedPlan(plan.id)}>
                    ...
                    {/* Plan button */}
                    <div className="mb-8">
                      <Button className={`w-full h-14 text-base rounded-xl font-medium ${selectedPlan === plan.id
                ? 'bg-primary hover:bg-primary/90 text-primary-foreground'
                : plan.popular
                    ? 'bg-primary hover:bg-primary/90 text-primary-foreground'
                    : 'bg-muted hover:bg-muted/80 text-foreground border border-border'}`} onClick={(e) => {
                e.stopPropagation();
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {isFirstLogin && (<Button variant="outline" onClick={handleSkip} className="h-12 px-8 bg-transparent border-border text-muted-foreground hover:bg-muted hover:text-foreground text-base font-medium">
                  Skip for now
                </Button>)}
              <Button onClick={handleGetStarted} disabled={!selectedPlan && !isChannelConnection} className="h-12 px-8 bg-primary hover:bg-primary/90 text-primary-foreground gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-base font-medium">
