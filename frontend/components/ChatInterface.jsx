...
import { Sparkles, Plus, Mic, ChevronDown, ChevronUp, Home, Stethoscope, Eye, Building, Thermometer, Car, Utensils, Briefcase, HeartHandshake, GraduationCap, Dumbbell, Scissors, ShoppingBag, Wrench, Palette, Shield, Truck, Laptop, Trees, Workflow, Link, Mail, Youtube, Facebook, Instagram, Search, FileSpreadsheet, ImageIcon, Bot, Users, Paperclip, Factory, ArrowUp } from 'lucide-react';
...
export function ChatInterface({ onCampaignGenerated, onShowOnboarding, isAuthenticated, onRequireAuth }) {
    ...
    const handleNicheSelect = (prompt) => {
    ...
    return (<div className="h-full flex flex-col bg-background">
      {messages.length === 0 ? (
        ...
        isMobile ? (<div className="h-full flex flex-col">
            {/* Centered Message */}
            ...
            <div className="px-4 pb-4">
              <Collapsible open={industriesExpanded} onOpenChange={setIndustriesExpanded}>
                <CollapsibleTrigger asChild>
                  <Button variant="ghost" className="w-full justify-between p-3 h-auto text-left bg-muted/30 hover:bg-muted/50 rounded-lg">
                    <span className="text-sm text-muted-foreground">Select Industry</span>
                    {industriesExpanded ? (<ChevronUp className="h-4 w-4 text-muted-foreground"/>) : (<ChevronDown className="h-4 w-4 text-muted-foreground"/>)}
                  </Button>
                </CollapsibleTrigger>
                ...
                return (<Button key={niche.id} variant="ghost" className="h-16 px-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg flex flex-col gap-1 border border-border/30 hover:border-border" onClick={() => {
            ...
            <div className="border-t border-border bg-background/80 backdrop-blur-sm p-4">
              <div className="relative">
                <Input value={inputValue} onChange={handleInputChange} onKeyPress={handleKeyPress} placeholder="Ask anything" className="w-full h-12 pl-4 pr-12 rounded-full border border-border/50 bg-muted/30 placeholder:text-muted-foreground/70 focus-visible:ring-0 focus-visible:ring-offset-0" disabled={isTyping}/>
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                  <Button variant="ghost" size="sm" className="h-8 w-8 rounded-full p-0" disabled={isTyping}>
                    <Mic className="h-4 w-4"/>
                  </Button>
                  <Button size="sm" className={`h-8 w-8 rounded-full p-0 ${inputValue.trim() && !isTyping ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-muted/50 hover:bg-muted text-muted-foreground'}`} onClick={handleSend} disabled={!inputValue.trim() || isTyping}>
              ...
              <div className="flex items-center gap-2 mt-3">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-sm bg-muted/50 hover:bg-muted border border-border/50">
                      <Plus className="h-3 w-3 mr-2"/>
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-64 p-1 bg-muted/90 backdrop-blur-sm border border-border/50" align="start">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-sm bg-muted/50 hover:bg-muted border border-border/50 min-w-0 flex-1">
                      {(() => {
                ...
                return <SelectedChannelIcon className="h-3 w-3 mr-2 flex-shrink-0"/>;
                  ...
                  <DropdownMenuContent className="w-80 p-1 bg-muted/90 backdrop-blur-sm border border-border/50" align="start">
                    {channels.map((channel) => {
                const IconComponent = channel.icon;
                return (<DropdownMenuItem key={channel.id} className="flex items-center justify-between p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => handleChannelSelect(channel.name)}>
                          <div className="flex items-center gap-3">
                            <IconComponent className="h-4 w-4 text-muted-foreground"/>
                            <span className="text-sm">{channel.name}</span>
                          </div>
                          {channel.connected ? (<span className="text-xs text-green-500 font-medium">Connected</span>) : (<span className="text-xs text-muted-foreground">Connect</span>)}
                        </DropdownMenuItem>);
            })}
                    <DropdownMenuSeparator className="my-1"/>
                    <DropdownMenuItem className="flex items-center gap-3 p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => setSelectedChannel(null)}>
                      <Workflow className="h-4 w-4 text-muted-foreground"/>
            ...
            <div className="w-full max-w-4xl">
              <div className="relative">
                <div className="relative bg-muted/30 rounded-2xl sm:rounded-3xl border border-border/50 p-4 sm:p-6">
                  <Input value={inputValue} onChange={handleInputChange} onKeyPress={handleKeyPress} placeholder="Search connected sources" className="border-0 bg-transparent text-lg sm:text-xl h-12 sm:h-16 placeholder:text-muted-foreground/70 focus-visible:ring-0 focus-visible:ring-offset-0 pr-14" disabled={isTyping}/>
                  
                  {/* Send Button */}
                  <div className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2">
                    <Button size="sm" className={`h-8 w-8 sm:h-10 sm:w-10 rounded-full p-0 ${inputValue.trim() && !isTyping ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-muted/50 hover:bg-muted text-muted-foreground'}`} onClick={handleSend} disabled={!inputValue.trim() || isTyping}>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-3 sm:mt-4">
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="sm" className="h-8 rounded-full px-3 text-sm bg-muted/50 hover:bg-muted border border-border/50 min-w-0">
                          {(() => {
                const SelectedChannelIcon = getSelectedChannelIcon();
                return <SelectedChannelIcon className="h-3 w-3 mr-2 flex-shrink-0"/>;
            })()}
                ...
                const IconComponent = niche.icon;
                return (<DropdownMenuItem key={niche.id} className="flex items-center gap-3 p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => {
                        ...
                        <DropdownMenuSeparator className="my-1"/>
                        <DropdownMenuItem className="flex items-center gap-3 p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => setSelectedIndustry(null)}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6 gap-2 sm:gap-3 mt-6 sm:mt-8 max-w-4xl mx-auto">
                ...
                const IconComponent = niche.icon;
                return (<Button key={niche.id} variant="ghost" className="h-14 sm:h-16 px-2 sm:px-3 text-xs sm:text-sm text-muted-foreground hover:text-foreground hover:bg-muted rounded-xl sm:rounded-2xl flex flex-col gap-1 sm:gap-2 border border-border/30 hover:border-border" onClick={() => handleNicheSelect(niche.prompt)}>
              ...
              <div className="flex items-center justify-center mt-4">
                <Button variant="ghost" className="h-9 sm:h-10 px-4 sm:px-6 text-sm text-muted-foreground hover:text-foreground hover:bg-muted rounded-full gap-2">
        ...
        <div className="flex-1 flex flex-col">
          <ScrollArea className="flex-1 p-4 sm:p-6">
            <div className="space-y-4 sm:space-y-6 max-w-4xl mx-auto">
              {messages.map((message) => (<div key={message.id} className={`flex gap-3 sm:gap-4 ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {message.type === 'assistant' && (<div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-black dark:bg-white flex items-center justify-center flex-shrink-0">
                  ...
                  {message.type === 'user' && (<div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-muted flex items-center justify-center flex-shrink-0">
                      <span className="text-xs sm:text-sm font-medium">MT</span>
                    </div>)}
                </div>))}

              {isTyping && (<div className="flex gap-3 sm:gap-4 justify-start">
