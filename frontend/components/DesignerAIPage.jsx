import { Sparkles, ArrowUp, ArrowUpRight, ArrowDownRight, Eye, Download, Share2, Copy, Trash2, Edit, Grid, List, Plus, Upload, MoreVertical, Heart, FolderOpen, ChevronDown, Palette, Maximize, Paperclip, FileSpreadsheet, ImageIcon, Monitor, Building, Utensils, Globe, Facebook, Instagram, Youtube, Linkedin, Twitter, Search, FileImage, TrendingUp } from 'lucide-react';
...
export function DesignerAIPage() {
    ...
    const getFilteredSizes = () => {
        ...
        return DESIGN_SIZES.filter(size => selectedPlatformNames.some(platformName => size.platform.includes(platformName)));
    };
    ...
    const handlePlatformToggle = (platformId) => {
        ...
        const newFilteredSizes = newSelectedPlatforms.length === 0
            ? DESIGN_SIZES
            : DESIGN_SIZES.filter(size => newSelectedPlatforms.some(id => size.platform.includes(platformIdToName[id])));
      ...
      <div className="space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="designer" className="space-y-4">
                ...
                <div className="w-full max-w-4xl">
                  <div className="relative">
                    <div className="relative bg-muted/30 rounded-2xl sm:rounded-3xl border border-border/50 p-4 sm:p-6">
                      <Input value={designerInputValue} onChange={(e) => setDesignerInputValue(e.target.value)} placeholder="Create a modern coffee shop banner with warm colors and steam rising from a cup..." className="border-0 bg-transparent text-lg sm:text-xl h-12 sm:h-16 placeholder:text-muted-foreground/70 focus-visible:ring-0 focus-visible:ring-offset-0 pr-14" disabled={isDesignerTyping}/>
                      
                      {/* Send Button */}
                      <div className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2">
                        <Button size="sm" className={`h-8 w-8 sm:h-10 sm:w-10 rounded-full p-0 ${designerInputValue.trim() && !isDesignerTyping ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-muted/50 hover:bg-muted text-muted-foreground'}`} disabled={!designerInputValue.trim() || isDesignerTyping}>
            ...
            : selectedPlatforms.length === DESIGN_PLATFORMS.length
                ...
                : `${selectedPlatforms.length} Platform${selectedPlatforms.length > 1 ? 's' : ''}`}
                          ...
                          <DropdownMenuContent className="w-80 p-1 bg-muted/90 backdrop-blur-sm border border-border/50" align="start">
                            ...
                            <DropdownMenuItem className="flex items-center gap-3 p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => {
            if (selectedPlatforms.length === DESIGN_PLATFORMS.length) {
                ...
                setSelectedPlatforms(DESIGN_PLATFORMS.map(platform => platform.id));
                              ...
                              <Checkbox checked={selectedPlatforms.length === DESIGN_PLATFORMS.length} onChange={() => { }} className="pointer-events-none"/>
            ...
            return (<DropdownMenuItem key={platform.id} className="flex items-center justify-between p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => handlePlatformToggle(platform.id)}>
                                  <div className="flex items-center gap-3">
                                    <Checkbox checked={selectedPlatforms.includes(platform.id)} onChange={() => { }} className="pointer-events-none"/>
                                    <IconComponent className="h-4 w-4 text-muted-foreground"/>
                                    <span className="text-sm">{platform.name}</span>
                                  </div>
                                  {platform.connected ? (<span className="text-xs text-green-500 font-medium">Connected</span>) : (<span className="text-xs text-muted-foreground">Connect</span>)}
                                </DropdownMenuItem>);
        })}
                          </DropdownMenuContent>
                        </DropdownMenu>
            ...
            if (selectedSizes.length === filteredSizes.length) {
                ...
                setSelectedSizes(filteredSizes.map(size => size.id));
                              ...
                              <Checkbox checked={selectedSizes.length === filteredSizes.length && filteredSizes.length > 0} onChange={() => { }} className="pointer-events-none"/>
                            ...
                            {filteredSizes.map((size) => (<DropdownMenuItem key={size.id} className="flex items-center justify-between p-3 rounded-md hover:bg-accent/50 cursor-pointer" onClick={() => {
                if (selectedSizes.includes(size.id)) {
                    ...
                    setSelectedSizes([...selectedSizes, size.id]);
                                ...
                                <div className="flex items-center gap-3">
                                  <Checkbox checked={selectedSizes.includes(size.id)} onChange={() => { }} className="pointer-events-none"/>
                                  <div>
            ...
            return (<Card key={index} className="p-4 cursor-pointer hover:bg-muted/50 transition-colors border border-border/50" onClick={() => setDesignerInputValue(suggestion.prompt)}>
          ...
          <TabsContent value="library" className="space-y-4">
            ...
            return (<Card key={stat.label} className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className={`text-sm flex items-center gap-1 ${stat.changeType === 'positive' ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center border rounded-md">
                    <Button variant={libraryView === 'grid' ? 'default' : 'ghost'} size="sm" onClick={() => setLibraryView('grid')} className="rounded-r-none">
                      <Grid className="h-4 w-4"/>
                    </Button>
                    <Button variant={libraryView === 'list' ? 'default' : 'ghost'} size="sm" onClick={() => setLibraryView('list')} className="rounded-l-none">
            ...
            {/* Bulk Actions */}
            {selectedAssets.length > 0 && (<Card className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
            ...
            {/* Assets Grid/List */}
            {libraryView === 'grid' ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {LIBRARY_ASSETS.map((asset) => (<Card key={asset.id} className="group overflow-hidden">
                    <div className="relative">
                      {/* Thumbnail */}
                      <div className="aspect-video bg-muted relative overflow-hidden">
                        <img src={`https://images.unsplash.com/photo-${1500000000000 + asset.id}?w=400&h=300&fit=crop`} alt={asset.name} className="w-full h-full object-cover"/>
                        ...
                        <div className="absolute top-2 left-2 flex items-center gap-1">
                          ...
                          {asset.aiGenerated && (<Badge className="text-xs bg-blue-600 text-white">
                        ...
                        {/* Favorite */}
                        <Button variant="ghost" size="sm" className="absolute bottom-2 right-2 h-6 w-6 p-0">
                          <Heart className={`h-3 w-3 ${asset.favorite ? 'fill-red-500 text-red-500' : 'text-white'}`}/>
                        </Button>

                        {/* Selection Checkbox */}
                        <div className="absolute bottom-2 left-2">
                          <Checkbox checked={selectedAssets.includes(asset.id)} onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      <div className="p-3">
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {asset.tags.slice(0, 3).map((tag, index) => (<Badge key={index} variant="secondary" className="text-xs px-1 py-0">
                              {tag}
                            </Badge>))}
                          {asset.tags.length > 3 && (<Badge variant="secondary" className="text-xs px-1 py-0">
                              +{asset.tags.length - 3}
                            </Badge>)}
                        </div>
                      </div>
        ...
        <div className="space-y-2">
                {LIBRARY_ASSETS.map((asset) => (<Card key={asset.id} className="p-4">
                    ...
                    if (checked) {
                      ...
                      <div className="w-16 h-12 bg-muted rounded overflow-hidden flex-shrink-0">
                        <img src={`https://images.unsplash.com/photo-${1500000000000 + asset.id}?w=64&h=48&fit=crop`} alt={asset.name} className="w-full h-full object-cover"/>
                      </div>
            ...
            {/* Empty State */}
            {LIBRARY_ASSETS.length === 0 && (<Card className="p-12">
                <div className="text-center">
